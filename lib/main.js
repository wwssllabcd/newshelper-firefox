(function(){
  ...
  check_report = function(title, url, cb){
    ...
    return get_newshelper_db(function(opened_db){
		...
		return get_request.onsuccess = function(){
			...
			if (get_request.result) {
				newshelperWidget.tooltip = "DB_Title=" + get_request.result.news_title + "abc";
			}else{
				newshelperWidget.tooltip = "DB not found";
			}
			...
			var tiReq = transaction.objectStore('report').index('news_title').get(mod_t);
			
			return tiReq.onsuccess = function(){
				newshelperWidget.tooltip = "def";
				...
				if (tiReq.result && !parseInt(tiReq.result.deleted_at, 10)) {
					return cb(tiReq.result);
				}
  ...
  var replace_chtSign = function(src){
	src = src.replace("．",".");
	src = src.replace("！","!");
	src = src.replace("：",":");
	return src;
