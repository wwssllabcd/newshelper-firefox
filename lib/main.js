(function(){
      ...
      return get_request.onsuccess = function(){
        if (get_request.result && !parseInt(get_request.result.deleted_at, 10)) {
          return cb(get_request.result);
			...
			title = modifyWebsideTitle(title);
			index = objectStore.index('news_title');
			get_request = index.get(title);
  ...
  var modifyWebsideTitle = function(title){
  ...
  var removeYahoo = function(title){
	...
	var startIdx = title.indexOf(keyword);
  ...
  var isFind = function(db, keyword, field){
		var res;
		var index = db.index(field);
		var get_request = index.get(keyword);	
