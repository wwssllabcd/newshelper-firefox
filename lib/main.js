(function(){
  ...
  var searchByTitle = function(opened_db, title){
	
		var transaction, objectStore, index, get_request;
		transaction = opened_db.transaction('report', 'readonly');
		objectStore = transaction.objectStore('report');
		index = objectStore.index('news_title');
		
		var mod_t = modifyTitle(title);
		get_request = index.get(mod_t);
		...
		return get_request.onsuccess = function(){
			...
			res =  mod_t + ".." ;
			res +=  "\r\n" + title;
			res +=  "\r\n" + get_request.result.id;
			res +=  "\r\n" + get_request.result.news_title + ".." ;
			res +=  "\r\n" + get_request.result.news_link;
	
			newshelperWidget.tooltip = res;
      ...
      return get_request.onsuccess = function(){
        ...
        if (get_request.result && !parseInt(get_request.result.deleted_at, 10)) {
          return cb(get_request.result);
			...
			var titleRequest = searchByTitle(opened_db, title);
			
			console.log("=== aaa ===");
			console.log(url);
			console.log(titleRequest.result);
			console.log(titleRequest.result.deleted_at);
			
			if (titleRequest.result && !parseInt(titleRequest.result.deleted_at, 10)) {
				return cb(titleRequest.result);
			}
		}
  ...
  var modifyTitle = function(title){
	title = removeYahoo(title);
	title = removeOnePageNews(title);
	title = removeCW(title);
	title = remove_ChinaTime(title);
	return title;
  }

  var removeYahoo = function(title){
	var keyword = " - Yahoo奇摩新聞";
	return removeEnd(keyword, title);
  }
  
  var removeOnePageNews = function(title){
	var keyword = " - 一頁新聞/一頁.com";
	return removeEnd(keyword, title);
  }
  
  var removeCW = function(title){
	var keyword = " - 天下雜誌";
	return removeEnd(keyword, title);
  }
  
  var remove_ChinaTime = function(title){
	var keyword = " - 中時電子報";
	return removeEnd(keyword, title);
  }
  
   var removeEnd = function(keyword, src){
	var title = src;
	var startIdx = src.indexOf(keyword);
